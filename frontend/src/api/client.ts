...
export const api = {
  ...
  listNotes: (page = 1, limit = 20, tag?: string, pageId?: string) =>
    request(
      `/notes?page=${page}&limit=${limit}${tag ? `&tag=${tag}` : ""}${pageId ? `&page_id=${pageId}` : ""}`
    ),
  ...
  listEdges: (pageId?: string, noteId?: string) =>
    request(
      `/edges?${pageId ? `page_id=${pageId}` : ""}${noteId ? `&note_id=${noteId}` : ""}`
    ),
  createEdge: (data: {
    source_id: string
    target_id: string
  ...
  search: (q: string, limit = 10, pageId?: string) =>
    request(
      `/search?q=${encodeURIComponent(q)}&limit=${limit}${pageId ? `&page_id=${pageId}` : ""}`
    ),
  ...
  // ─── Curator ────────────────────────────────────
  curatorScan: () => request("/curator/scan", { method: "POST" }),
  curatorApply: (action: {
    action_type: string
  ...
  // ─── Health ─────────────────────────────────────
  health: () =>
    fetch(`${API_BASE.replace("/api", "")}/health`, { signal: AbortSignal.timeout(5000) })
      .then((r) => r.json())
      .catch(() => null),
}