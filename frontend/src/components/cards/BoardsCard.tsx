...
export function BoardsCard({ data }: Props) {
  const { send } = useChat()
  ...
  if (!data.boards.length) {
    return (
      <div className="glass-card rounded-2xl p-4 text-center">
        ...
        <p className="text-xs text-[var(--glass-text-dim)]">No boards yet</p>
      </div>
    )
  }
    ...
    <div className="space-y-1.5 stagger-children">
      {data.boards.map((b) => (
        <button
          ...
          className="glass-card w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-left group"
        >
