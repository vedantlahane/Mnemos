    ...
    <div className="space-y-1.5 stagger-children">
      {data.results.map((r) => (
        <button
          ...
          className="glass-card w-full rounded-2xl px-4 py-3 flex items-center gap-3 text-left cursor-pointer"
        >
            ...
            {r.tags.length > 0 && (
              <div className="flex gap-1 mt-1">
                {r.tags.slice(0, 3).map((t) => (
                  <span key={t} className="text-[10px] text-[var(--accent-light)]/60">#{t}</span>
                ))}
              </div>
            )}
          </div>
