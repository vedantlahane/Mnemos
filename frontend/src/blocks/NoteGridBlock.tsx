...
export default function NoteGridBlock({ item }: { item: StreamItem }) {
  ...
  if (notes.length === 0) {
    return (
      <div className="text-center text-[12px] text-[var(--glass-text-muted)] py-12 glass-surface-1 rounded-xl">
        {item.metadata?.tag
  ...
  return (
    <div>
      <div className="text-[10px] uppercase font-bold tracking-widest text-[var(--glass-text-muted)] mb-3">
        ...
        <span className="ml-2 text-[var(--glass-text-dim)]">({notes.length})</span>
      </div>
            ...
            {note.tags.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {note.tags.slice(0, 3).map((t) => (
                  <span
                    key={t}
                    className="flex items-center gap-1 text-[9px] bg-[rgba(99,102,241,0.08)] text-[var(--accent)] px-1.5 py-0.5 rounded uppercase tracking-wider font-semibold"
                  >
                    <Tag size={7} /> {t}
                  </span>
                ))}
