  ...
  const sections = [
    ...
    },
    {
      ...
      cmds: [
        ...
        { name: "/layout", desc: "Auto-reorganize" },
  ...
  return (
    <div className="glass rounded-2xl p-5 relative overflow-hidden">
      <div className="relative z-10">
        <div className="text-[9px] uppercase tracking-[0.2em] font-bold text-[var(--glass-text-muted)] mb-4">Commands</div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {sections.map((sec) => (
            <div key={sec.title}>
              <div className="text-[10px] uppercase tracking-widest font-bold text-[var(--accent-light)] mb-2">{sec.title}</div>
              {sec.cmds.map((c) => (
                <div key={c.name} className="flex justify-between py-1 group">
                  <div className="flex items-center gap-1.5">
                    <span className="font-mono text-[12px] text-[var(--glass-text)]">{c.name}</span>
                    {c.args && <span className="text-[10px] text-[var(--glass-text-muted)] italic">{c.args}</span>}
                  </div>
                  <span className="text-[10px] text-[var(--glass-text-dim)] group-hover:text-[var(--glass-text)] transition-colors">{c.desc}</span>
                </div>
              ))}
            </div>
          ))}
        </div>
        <div className="mt-5 p-3 rounded-xl bg-[rgba(99,102,241,0.05)] border border-[rgba(99,102,241,0.1)]">
          <p className="text-[11px] text-[var(--glass-text-dim)] leading-relaxed">
            Everything without <span className="font-mono text-[var(--accent-light)]">/</span> is sent as a natural language question.
            Press <kbd className="font-mono bg-[rgba(255,255,255,0.06)] px-1.5 py-0.5 rounded text-[10px] text-white mx-0.5">⌘K</kbd> to focus.
          </p>
        </div>
      </div>
    </div>
