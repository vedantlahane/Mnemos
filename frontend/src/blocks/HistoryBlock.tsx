    ...
    <div className="glass-surface-1 p-6 rounded-2xl">
      ...
      {Object.entries(grouped).map(([date, convos]) => (
        <div key={date} className="mb-4">
          <div className="text-[11px] text-[var(--glass-text-muted)] font-semibold mb-2">{date}</div>
          {convos.map((c) => (
            <div
              key={c.id}
            ...
            >
              ...
              <button
                ...
                className="text-[var(--glass-text-muted)] hover:text-[var(--red)] transition-colors p-1"
              >
                <Trash2 size={13} />
              </button>
            </div>
