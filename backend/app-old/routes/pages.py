...
def _normalize_view_mode(view_mode: str) -> str:
    mode = (view_mode or "canvas").strip().lower()
    if mode not in {"canvas", "notebook"}:
        raise HTTPException(status_code=400, detail="view_mode must be 'canvas' or 'notebook'")
    return mode
...
async def get_page_canvas(
    page_id: str,
    view_mode: str = "canvas",
    ...
    cache_key = page_id if mode == "canvas" else f"{page_id}:notebook"

    canvas = await cache_svc.get_canvas_cached(
        cache_key,
        fetcher=lambda: db.get_page_canvas(page_id, user_id=user_id, view_mode=mode),
    )
    return canvas


@router.put("/pages/{page_id}/canvas")
async def save_page_canvas(
    page_id: str,
    payload: dict,
    view_mode: str = "canvas",
    user_id: str = Depends(get_optional_user_id),
    ...
    mode = _normalize_view_mode(view_mode)
    scene_key = "notebook_data" if mode == "notebook" else "canvas_data"

    updates = {}
    if "viewport" in payload:
        updates["viewport"] = payload["viewport"]
    if "canvas_data" in payload:
        updates[scene_key] = payload["canvas_data"] or {}
    ...
    if not updates:
        raise HTTPException(status_code=400, detail="viewport or scene data required")

    await db.update_page(page_id, user_id=user_id, **updates)
    await cache_svc.invalidate_canvas(page_id)
    await cache_svc.invalidate_canvas(f"{page_id}:notebook")

    # Sync positions from scene → DB (scene is authority)
    try:
        scene = updates.get(scene_key)
        if scene and scene_key == "canvas_data":
