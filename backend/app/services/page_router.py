...
async def route_note(text: str, title: str = None, tags: list[str] = None,
    ...
    try:
        ...
        if not uncat:
            uncat = await db.insert_page(name="Uncategorized", user_id=user_id)
        return {"page_id": uncat["id"], "page_name": "Uncategorized", "confidence": 0.0, "reason": f"LLM failed: {e}"}
    ...
    if not page:
        page = await db.get_page_by_name("Uncategorized", user_id=user_id)
        if not page:
            page = await db.insert_page(name="Uncategorized", user_id=user_id)
        page_name = "Uncategorized"
